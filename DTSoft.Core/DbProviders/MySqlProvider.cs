...
namespace DTSoft.Core.DbProviders
{
    public class MySqlProvider : IDbProvider
    {
        ...
        public MySqlProvider(string? databaseName = null, ITextLocalizer? localizer = null)
        {
        ...
        {
            var dbName = DatabaseName ?? throw new InvalidOperationException(DbProviderMessages.Text(_localizer, "db.databaseNameRequiredForTableExists"));
            
            // 验证数据库名和表名
            if (!IdentifierPattern.IsMatch(dbName))
                throw new ArgumentException(DbProviderMessages.Text(_localizer, "db.databaseNameInvalid", dbName), nameof(dbName));
            if (!IdentifierPattern.IsMatch(tableName))
                throw new ArgumentException(DbProviderMessages.Text(_localizer, "db.tableNameInvalid", tableName), nameof(tableName));
        ...
        {
            var dbName = DatabaseName ?? throw new InvalidOperationException(DbProviderMessages.Text(_localizer, "db.databaseNameRequiredForColumns"));
        ...
        private static int NormalizeColumnLength(FieldConfig field, int defaultLength, int? existingMaxLength = null)
        {
            ...
            return Math.Clamp(length, 1, 65535);
        }
        ...
        public string BuildCreateTableSql(string tableName, List<FieldConfig> fields)
        {
            ...
            sql += $"{QuoteColumnName(MicroTableSystemColumns.CreatedTime)} DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, ";
            sql += $"{QuoteColumnName(MicroTableSystemColumns.UpdatedTime)} DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, ";
        ...
        public string BuildSelectWithPagingSql(string tableName, List<FieldConfig> fields, string whereClause, string orderByClause, int pageNum, int pageSize)
        {
            ...
            return $"SELECT {selectFields} FROM {QuoteTableName(tableName)}{whereClause} {orderByClause} LIMIT {offset}, {pageSize}";
        }

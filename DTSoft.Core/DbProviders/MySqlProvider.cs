...
namespace DTSoft.Core.DbProviders
{
    public class MySqlProvider : IDbProvider
    {
        ...
        public string BuildCreateTableSql(string tableName, List<FieldConfig> fields)
        {
            ...
            sql += $"{QuoteColumnName("Id")} BIGINT PRIMARY KEY, ";
            sql += $"{QuoteColumnName("CreateTime")} DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, ";
            sql += $"{QuoteColumnName("UpdateTime")} DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, ";
        ...
        public string BuildSelectWithPagingSql(string tableName, List<FieldConfig> fields, string keyword, int pageNum, int pageSize)
        {
            ...
            return $"SELECT {selectFields} FROM {QuoteTableName(tableName)}{where} ORDER BY {QuoteColumnName("Id")} ASC LIMIT {offset}, {pageSize}";
        }
